...
const humanConfig: Partial<Config> = { // user configuration for human
  modelBasePath: 'https://cdn.jsdelivr.net/npm/@vladmandic/human/models/', // load models from cdn
  filter: { enabled: false },
  face: {
    enabled: true, detector: { rotation: false }, mesh: { enabled: true }, iris: { enabled: true }, description: { enabled: true }, emotion: { enabled: true },
  },
  body: { enabled: false },
  hand: { enabled: false },
...
human.draw.options.font = 'small-caps 18px "Segoe UI"'; // set font used to draw labels when using draw methods
...
async function webCam() { // initialize webcam
  ...
  log('video:', dom.video.videoWidth, dom.video.videoHeight, track.label, {
    stream, track, settings, constraints, capabilities,
  });
